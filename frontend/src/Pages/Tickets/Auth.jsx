...
// Customer-only route component - for pages that only customers should access
const CustomerOnlyRoute = ({ children }) => {
  ...
  // Check if user is a customer (not ADMIN, ENGINEER, or MANAGER)
  if (user && (user.role === 'ADMIN' || user.role === 'ENGINEER' || user.role === 'MANAGER')) {
    return (
      <div className="auth-message">
        <div className="auth-message-content">
          <div className="lock-icon">
          ...
          <h2>Access Denied</h2>
          <p>This area is for customers only. Staff members should use the internal support system.</p>
          <div className="auth-buttons">
            <Link to="/" className="auth-button primary">Go to Home</Link>
          </div>
        </div>
...
export { AdminRoute, ProtectedRoute, EngineerRoute, ManagerRoute, CustomerOnlyRoute };
